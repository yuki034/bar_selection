export const dummyRecipes = [
  {
    id: 1,
    name: "ジントニック",
    base: "ジン",
    ingredients: ["ジン 45ml", "トニックウォーター 適量", "ライム 1/4個"],
    image: "https://www.thecocktaildb.com/images/media/drink/qcgz0t1643821443.jpg",
    description: "ジンとトニックウォーターの爽やかな味わいが特徴の定番カクテル。ライムを添えてさっぱりと楽しめます。",
  },
  {
    id: 2,
    name: "モヒート",
    base: "ラム",
    ingredients: ["ラム 45ml", "ミント 10枚", "ライム 1/2個", "ソーダ 適量", "砂糖 2tsp"],
    image: "https://www.thecocktaildb.com/images/media/drink/metwgh1606770327.jpg",
    description: "ミントとライムの香りが爽快な、夏にぴったりのキューバ生まれのカクテル。",
  },
  {
    id: 3,
    name: "カシスオレンジ",
    base: "リキュール",
    ingredients: ["カシスリキュール 30ml", "オレンジジュース 90ml"],
    image: "https://cocktailrecipe.jp/img/card/cassisOrange.WebP",
    description: "カシスリキュールの甘さとオレンジジュースの酸味が絶妙にマッチした人気カクテル。",
  },
  {
    id: 4,
    name: "スクリュードライバー",
    base: "ウォッカ",
    ingredients: ["ウォッカ 45ml", "オレンジジュース 90ml"],
    image: "https://www.thecocktaildb.com/images/media/drink/8xnyke1504352207.jpg",
    description: "ウォッカとオレンジジュースだけで作れるシンプルで飲みやすいカクテル。",
  },
  {
    id: 5,
    name: "マルガリータ",
    base: "テキーラ",
    ingredients: ["テキーラ 45ml", "コアントロー 20ml", "ライムジュース 30ml", "塩 適量"],
    image: "https://www.thecocktaildb.com/images/media/drink/5noda61589575158.jpg",
    description: "テキーラベースの代表的なカクテル。グラスの縁に塩をつけて飲むのが特徴です。",
  },
  {
    id: 6,
    name: "ハイボール",
    base: "ウイスキー",
    ingredients: ["ウイスキー 45ml", "ソーダ 適量", "レモン 1/4個"],
    image: "https://prcdn.freetls.fastly.net/release_image/30117/1336/30117-1336-bc22b77cda548a3e2f3b69b3b9e25ca7-600x400.jpg?format=jpeg&auto=webp&fit=bounds&width=720&height=480",
    description: "ウイスキーをソーダで割った、日本でも人気の高い爽快なカクテル。",
  },
  {
    id: 7,
    name: "キール",
    base: "ワイン",
    ingredients: ["白ワイン 90ml", "カシスリキュール 30ml"],
    image: "https://www.thecocktaildb.com/images/media/drink/apneom1504370294.jpg",
    description: "白ワインとカシスリキュールを合わせた、フランス生まれの上品なカクテル。",
  },
  {
    id: 8,
    name: "シャンディガフ",
    base: "ビール",
    ingredients: ["ビール 150ml", "ジンジャーエール 150ml"],
    image: "data:image/jpeg;base64,",
    description: "ビールとジンジャーエールを同量で割った、飲みやすく爽やかなカクテル。",
  },
  {
    id: 9,
    name: "グラスホッパー",
    base: "リキュール",
    ingredients: ["ホワイトクリームドミント 30ml", "ホワイトクリームドカカオ 30ml", "生クリーム 30ml"],
    image: "https://img.freepik.com/premium-photo/grasshopper-drink-with-cherr-isolated-black-backgroudn_149482-31.jpg?w=360",
    description: "ミントリキュールとカカオリキュール、生クリームの甘くてデザート感覚のカクテル。",
  },
  {
    id: 10,
    name: "ギムレット",
    base: "ジン",
    ingredients: ["ジン 45ml", "ライムジュース 30ml", "砂糖 1tsp"],
    image: "https://www.thecocktaildb.com/images/media/drink/3z6xdi1589574603.jpg",
    description: "ジンとライムジュースのシンプルな組み合わせ。大人の味わいが楽しめます。",
  },
  {
    id:11,
    name:"モスコミュール",
    base:"ウォッカ",
    ingredients:["ウォッカ 45ml","ライムジュース 10ml","ジンジャーエール 120ml"],
    image:"https://upload.wikimedia.org/wikipedia/commons/thumb/a/a4/Moscow_Mule_at_Rye%2C_San_Francisco.jpg/500px-Moscow_Mule_at_Rye%2C_San_Francisco.jpg",
    description: "ウォッカとジンジャーエールの刺激的な味わいが特徴のカクテル。",
  },
  {
    id:12,
    name:"マティーニ",
    base:"ジン",
    ingredients:["ジン 45ml","ベルモット 15ml","オリーブ 1個","レモンピール 1片"],
    image:"https://www.asahibeer.co.jp/cocktail/search/image/item/2000000181.jpg",
    description: "ジンとベルモットを合わせた、世界的に有名なスタンダードカクテル。",
  },
  {
    id:13,
    name:"ソルティドッグ",
    base:"ウォッカ",
    ingredients:["ウォッカ 45ml","グレープフルーツジュース 60ml","塩 少々"],
    image:"https://otokonokakurega.com/wp-content/uploads/2022/06/78805a221a988e79ef3f42d7c5bfd418-20.jpeg.webp",
    description:"爽やかなグレープフルーツジュースとウォッカを合わせた、すっきりとした味わいのカクテル。グラスの縁にまぶした塩が、ほのかな苦味と果実の甘酸っぱさを引き立て、クセになるバランスの一杯です。",
  },
  {
    id:14,
    name:"ブラッディマリー",
    base:"ウォッカ",
    ingredients:["ウォッカ 45ml","トマトジュース 90ml","レモンジュース 15ml","ウスターソース 2-3滴","タバスコ 2-3滴","セロリ 1本","塩・コショウ 少々"],
    image:"https://www.thecocktaildb.com/images/media/drink/t6caa21582485702.jpg",
    description:"ウォッカとトマトジュースをベースにした、スパイシーで栄養豊富なカクテル。朝食やブランチにぴったりです。",
  },
  {
    id:15,
    name:"ピニャコラーダ",
    base:"ラム",
    ingredients:["ラム 45ml","ココナッツミルク 30ml","パイナップルジュース 30ml","パイナップル 1片"],
    image:"https://www.thecocktaildb.com/images/media/drink/twyrpu1439907629.jpg",
    description:"ラムとココナッツミルク、パイナップルジュースを合わせた、トロピカルな味わいのカクテル。南国のリゾート気分を味わえます。",
  },
  {
    id:16,
    name:"カイピリーニャ",
    base:"カシャーサ",
    ingredients:["カシャーサ 45ml","ライム 1個","砂糖 2tsp","ミント 数枚"],
    image:"https://www.thecocktaildb.com/images/media/drink/cyevuw1568926341.jpg",
    description:"ブラジルの国酒カシャーサを使った、ライムとミントの爽やかなカクテル。サンバのリズムと共に楽しみたい一杯です。",
  },
  {
    id:17,
    name:"アペロールスプリッツ",
    base:"リキュール",
    ingredients:["アペロール 45ml","プロセッコ 45ml","ソーダ 15ml","オレンジ 1片"],
    image:"https://www.thecocktaildb.com/images/media/drink/iwk8b41504086329.jpg",
    description:"イタリア生まれのアペロールとプロセッコを合わせた、ビターで上品なカクテル。夕方のアペリティーボに最適です。",
  },
  {
    id:18,
    name:"オールドファッションド",
    base:"ウイスキー",
    ingredients:["ウイスキー 45ml","アンゴスチュラビターズ 2-3滴","砂糖 1tsp","オレンジピール 1片","チェリー 1個"],
    image:"https://www.thecocktaildb.com/images/media/drink/vrwquq1478252802.jpg",
    description:"ウイスキーをベースにした古典的なカクテル。ビターズと砂糖の絶妙なバランスで、大人の味わいを楽しめます。",
  },
  {
    id:19,
    name:"ベラス",
    base:"シャンパン",
    ingredients:["シャンパン 90ml","ピーチリキュール 30ml","ラズベリーリキュール 15ml","ラズベリー 2-3個"],
    image:"https://www.thecocktaildb.com/images/media/drink/qwvqpw1483174740.jpg",
    description:"シャンパンとピーチリキュール、ラズベリーリキュールを合わせた、華やかで上品なカクテル。特別な日の乾杯にぴったりです。",
  },
  {
    id:20,
    name:"ミモザ",
    base:"シャンパン",
    ingredients:["シャンパン 90ml","オレンジジュース 30ml","オレンジ 1片"],
    image:"https://www.thecocktaildb.com/images/media/drink/ctt20s1504373488.jpg",
    description:"シャンパンとオレンジジュースのシンプルな組み合わせ。朝食やブランチに人気の、爽やかで優雅なカクテルです。",
  },
  {
    id:21,
    name:"フレンチ75",
    base:"シャンパン",
    ingredients:["ジン 30ml","レモンジュース 15ml","砂糖 1tsp","シャンパン 60ml","レモンピール 1片"],
    image:"https://www.thecocktaildb.com/images/media/drink/4qmxrq1439907329.jpg",
    description:"ジンとシャンパンを合わせた、洗練された味わいのカクテル。第一次世界大戦中に考案された歴史あるカクテルです。",
  },
  {
    id:22,
    name:"アブサントフラッシュ",
    base:"スピリッツ",
    ingredients:["アブサント 30ml","冷水 適量","砂糖 1tsp"],
    image:"https://www.thecocktaildb.com/images/media/drink/yswuwp1441254553.jpg",
    description:"アブサントに冷水を注いで作る、伝統的な飲み方。水を注ぐことでアブサントが白濁する美しい現象を楽しめます。",
  },
  {
    id:23,
    name:"サンブーカ",
    base:"スピリッツ",
    ingredients:["サンブーカ 45ml","コーヒー豆 3粒","レモンピール 1片"],
    image:"https://www.thecocktaildb.com/images/media/drink/x8lhp41513703167.jpg",
    description:"イタリアの伝統的なリキュール。コーヒー豆を浮かべて飲むのが特徴で、食後のデザート酒として人気があります。",
  }
];